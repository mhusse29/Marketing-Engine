...
const ratingLabels = ["GOOD", "NOT BAD", "NOT GREAT", "BAD"];
const ratingValues = [2, 1, 1, 0]; // Maps to our 0-2 rating system

const Default = ({ label, touchpointType, contextData }: {
  label: string;
  touchpointType?: string;
  contextData?: Record<string, any>;
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedEmoji === null || !user) return;
    ...
    try {
      const response = await fetch('/api/feedback', {
        ...
        body: JSON.stringify({
          userId: user.id,
          touchpointType: touchpointType || 'feature_usage',
          rating: ratingValues[selectedEmoji],
          ratingLabel: ratingLabels[selectedEmoji],
          comments: feedbackText || null,
          contextData: { ...contextData, feedbackType: 'emoji-widget' },
      ...
      >
        {submitted ? (
          <div className="p-8 text-center">
            <div className="text-green-600 text-lg font-semibold">Thank you!</div>
            <p className="text-sm text-gray-700 mt-2">Your feedback helps us improve</p>
          </div>
        ) : (
...
const Inline = ({ label, touchpointType, contextData }: {
  label: string;
  touchpointType?: string;
  contextData?: Record<string, any>;
}) => {
    ...
    try {
      const response = await fetch('/api/feedback', {
        ...
        body: JSON.stringify({
          ...
          ratingLabel: ratingLabels[selectedEmoji],
          comments: feedbackText || null,
          contextData: { ...contextData, feedbackType: 'emoji-widget-inline' },
          pageUrl: window.location.href,
          userAgent: navigator.userAgent
        })
      });
    ...
    >
      {submitted ? (
        <div className="p-4 text-center">
          <div className="text-green-600 text-sm font-semibold">Thanks!</div>
        </div>
      ) : (
        <>
          ...
          {showContent && (
            <form className="w-full transition-opacity duration-300 opacity-100" onSubmit={handleSubmit}>
...
export const Feedback = ({ type = "default", label, touchpointType = "feature_usage", contextData }: FeedbackProps) => {
  return (type === "default" || type === undefined) 
    ? <Default label={label} touchpointType={touchpointType} contextData={contextData} /> 
    : <Inline label={label} touchpointType={touchpointType} contextData={contextData} />;
};
